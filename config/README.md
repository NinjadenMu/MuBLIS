A configuration defines which targets are linked into a MuBLIS library and
which one is selected at runtime. This separation allows one library to contain
multiple hardware targets while keeping ISA-specific code isolated in those
targets.

Configurations are discovered atuomatically from `config/{config name}/config.mk`, and may be selected with `make CONFIG={config name}`.
...
Note that feature detection must run safely on every platform supported by the configuration.  Note that this is why configs separate on OS - Mac, Linux, Windows, etc. expose different methods for detecting CPU features.

It may be smart to include `reference` as a fallback for broad hardware families.

### Config Build Rules
Create `config/<name>/config.mk` with a dispatch source (defining `mublis_get_context`) and at least one target (name of specific target subdirectory in `targets`).
